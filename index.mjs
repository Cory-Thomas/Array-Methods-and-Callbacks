...
function getYears( callback ) {
    return callback(fifaData).map( item => item["Year"] );
};
...
function getWinners( callback ) {
    ...
    // let winner = callback(fifaData).map( item => item["Win conditions"]);
    let winner = callback(fifaData).map( item => {
...
/* Task 7: Write a function called `getAverageGoals` that accepts a parameter `data` and returns the the average number of home team goals and away team goals 
scored per match (Hint: use .reduce and do this in 2 steps) */

function getAverageGoals( data ) {

    let home = data.reduce( (accumulator, item) => {
        return accumulator + item["Home Team Goals"];
    },0);
    home /= data.length;

    let away = data.reduce( (accumulator, item) => {
        return accumulator + item["Away Team Goals"];
    },0);
    away /= data.length;

    return `Home Team Average: ${home.toFixed(2)} :: Away Team Average: ${away.toFixed(2)}`;
};
